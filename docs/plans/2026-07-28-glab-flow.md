...
glab-flow/                                  # /Users/eliojin/IdeaProjects/glab-flow (new git repo)
  ...
  engine/
    ...
    src/
      ...
      guard.test.ts
      gitlab.ts                             # read issue (labels/body/comments, parse 交付协同) + applyWritePlan (retry)
      gitlab.test.ts
      render.ts                             # structured payload -> harness markdown templates
      render.test.ts
      cli.ts                                # subcommands: node / validate / render / plan / apply
    fixtures/
...
### Task 3.2: GitLab client — HTTP read + apply write plan with retry (TDD, mocked fetch)

**Files:**
- Modify: `engine/src/gitlab.ts`, `engine/src/gitlab.test.ts`

- [ ] **Step 1: Append failing tests (mock global fetch)**

```ts
import { fetchIssue, applyWritePlan } from './gitlab.js';
...
describe('fetchIssue', () => {
  it('GETs the issue and retries on network error', async () => {
    ...
    const got = await fetchIssue(base, token, 123);
...
describe('applyWritePlan', () => {
  it('translates ops to GitLab API calls', async () => {
    const calls: string[] = [];
    mockFetch((url, init) => { calls.push(`${init?.method ?? 'GET'} ${url}`); return new Response('{}', { status: 200 }); });
    await applyWritePlan(base, token, { issueIid: 123, ops: [
      { kind: 'remove_label', value: 'story-status::草稿中' },
      { kind: 'add_label', value: 'story-status::待评审' },
      { kind: 'set_assignee', username: '@pm' },
      { kind: 'add_comment', body: 'hi' },
    ] });
    expect(calls.some((c) => c.includes('/issues/123?') && c.startsWith('PUT'))).toBe(true);
    expect(calls.some((c) => c.includes('/issues/123/notes') && c.startsWith('POST'))).toBe(true);
  });
});
```
...
Run: `pnpm vitest run engine/src/gitlab.test.ts`
Expected: FAIL (fetchIssue/applyWritePlan undefined).

- [ ] **Step 3: Add HTTP functions to `engine/src/gitlab.ts`**
...
export async function fetchIssue(base: string, token: string, iid: number): Promise<GitLabIssue> {
  return http<GitLabIssue>(`${base}/projects/:id/issues/${iid}`.replace(':id', '3915'), token, {});
}

export async function applyWritePlan(base: string, token: string, plan: WritePlan): Promise<void> {
  const pid = 3915;
  ...
  if (addLabels.length || removeLabels.length || assignee || doClose) {
    ...
    await http(`${base}/projects/${pid}/issues/${plan.issueIid}`, token, { method: 'PUT', body: JSON.stringify(body) });
  }
  for (const c of comments) {
    await http(`${base}/projects/${pid}/issues/${plan.issueIid}/notes`, token, { method: 'POST', body: JSON.stringify({ body: c }) });
  }
}
```

> Note: project id `3915` is the `oa-ai-native-harness` project (from spec). Make it configurable via env `GLAB_FLOW_PROJECT_ID` in Task 4.1; hardcode default for now.
...
```bash
git add engine/src/gitlab.ts engine/src/gitlab.test.ts
git commit -m "feat(gitlab): fetchIssue + applyWritePlan with retry (mocked)"
```
...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'plan': {
      // stdin: { labels, payload } -> prints a validated WritePlan JSON (does not apply)
      const input = JSON.parse(readStdin()) as { labels: string[]; payload: import('./types.js').Payload };
    ...
    case 'apply': {
      const plan = JSON.parse(readStdin()) as import('./types.js').WritePlan;
      const guard = validateWritePlan(plan);
      if (!guard.ok) { console.error(JSON.stringify(guard)); process.exit(1); }
      await applyWritePlan(BASE, TOKEN, plan);
      console.log(JSON.stringify({ applied: true }));
      break;
    }
    case 'resolve-assignee': {
      // glab-flow resolve-assignee <iid> <role>  -> reads issue 交付协同 table, prints @user
      const iid = Number(args[0]); const role = args[1];
      const issue: GitLabIssue = await fetchIssue(`${BASE}/projects/${PROJECT}`, TOKEN, iid);
      const map = parseAssigneeTable(issue.description);
...
git commit -m "feat(cli): node/validate/render/plan/apply/resolve-assignee"
```
...
**引擎**：`/Users/eliojin/IdeaProjects/glab-flow`，命令前缀 `cd /Users/eliojin/IdeaProjects/glab-flow && pnpm cli <cmd>`。环境变量 `GLAB_FLOW_TOKEN`（GitLab token）、`GLAB_FLOW_PROJECT_ID`（默认 3915）。

## Leader 每轮编排（严格按序）

1. **读状态**：`pnpm cli resolve-assignee <iid> 产品` 之外，直接用 GitLab API（token）读 Issue 的 labels/description/comments；或用 engine `node` 命令得当前节点。
...
7. **应用**：`pnpm cli apply`（stdin: WritePlan）。
8. **下一节点**：循环到 `已完成` 或用户停。
...
Add sections: Architecture (pointer to spec + docs/architecture.md), Engine commands (`pnpm cli node|validate|render|plan|apply|resolve-assignee`), Env vars (`GLAB_FLOW_TOKEN`, `GLAB_FLOW_PROJECT_ID`, `GLAB_FLOW_API`), Install (`./install.sh`), Test (`pnpm test`), Relation to dev-flow (coexist; GitLab-native).
