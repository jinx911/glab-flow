...
- **环境维度二分(判定口诀)**:环境改变的是**执行参数**(base_url/账号/数据前缀)→ 同套件 `--environment` 切,不复制;环境改变的是**场景集合**(开发自测覆盖面 vs Stage 验收覆盖面不同)→ **分目录分套件**,环境词进目录/套件名是官方分组用法(如「00 本地开发自测 / 01 Stage 验收回归」)。
- **禁止的是**:同一逻辑**复制 N 份仅参数不同**的场景/套件(那意味着改用例要改 N 处)——这类一律收敛为一份 + `--environment` + 运行时变量。
...
- **目录规范(官方分组用法)**:场景目录按「需求 → 功能域 → (环境子目录,仅当场景集合分环境)」;套件目录按「需求域 → 用途(本地自测/Stage 验收/专项矩阵)」。场景/套件**不放根目录**——建了目录必须归位(`folder-id`),空目录及时删。
