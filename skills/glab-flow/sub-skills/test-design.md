...
- **节点归属**：「测试中」节点（`../nodes.md`），Leader Read 本文件内联执行或 spawn `general-purpose` 以本文件为 prompt。产出 `test-plan.md` 落 `.glab-flow/<iid>/spec/`。产出后 Leader 必须向**父 Issue**新增 `kind: test-plan` 的 `glab-flow:artifact-receipt:v1` 产物回执，回读、解析并写入 state 缓存后，才可标记「测试计划」完成或推进待发布。
