...
- **传入上下文**：把待评审的 diff、相关 spec（design.md 关键文件表）、当前 test-plan 与 local TestRun 证据一并喂给 reviewer，让它对齐规格而非凭空挑刺。**标准化材料用 `review-pack` 命令产出**：stdin 传与 `next` 同源的输入（labels/body/notes + du），返回 spec 路径、DU 内部证据摘要、门禁缺口和评审指令段——Leader 把包与 diff 一起注入 reviewer，不手工拼材料；该内部摘要不写入公共 Issue 评论。
