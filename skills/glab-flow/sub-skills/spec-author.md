...
### 设计前 grilling（architect 写 design.md 前必做一轮，防「产品没想的」漏到提测）

对已评审需求做前沿式追问，把设计决策组织成一棵树，**在写方案前问掉**而不是提测才发现：

- **每轮只问前沿**（前提已确定、不依赖其他未回答问题的问题），一次问完整个前沿；每题编号 + **附你的推荐答案**（基于代码证据），用户确认/否决即可。回答解锁下游问题，每轮重算前沿。
...
- **终止**：前沿为空才开写 design.md——不允许还有未问分支就落方案。追问及结论记入 design.md「设计决策」小节（每条：问题 → 推荐 → 用户裁定），评审可追溯。
...
- [ ] 设计前 grilling 已走完（前沿为空）：设计决策含「问题 → 推荐 → 用户裁定」记录，无未问分支
- [ ] 所有触发的工程章节存在且填写完整
