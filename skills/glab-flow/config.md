...
- `run_mode` —— `semi-auto`（默认）或 `full-auto`；仅是旧 state 与开发入口前的兼容默认值。进入「已评审 → 开发中」必须另行 `run-mode-select` 持久化 Issue 级选择，裸配置值不能授权自动写回；hard_gate 两模式都强制人工（见 `gate.md`）。
...
4. 解析成功 → 得到 `GlabConfig` JSON。从此对象派生后续编排所需的全部项目参数：
   ...
   - 可选项按需读取（`deployBranch` / `jenkins` / `databases` / `testEnvironments` / `apifox`）。
...
| `run_mode` | 可选 | `runMode` | `semi-auto`（默认）或 `full-auto`；仅旧 state/开发入口前兼容默认，不能替代不可变 `runModeSelection` |
...
| `jenkins.branch_param` | 可选 | `jenkins.branchParam` | 分支参数名，默认 `"branch"` |
...
| `databases.<name>.mcp` | 可选 | `databases.<name>.mcp` | 逻辑数据访问路由；不包含连接串或凭据 |
| `databases.<name>.desc` | 可选 | `databases.<name>.desc` | 数据库说明 |
| `test_environments.<name>.url` | 可选 | `testEnvironments.<name>.url` | 测试环境地址 |
| `test_environments.<name>.runtime` | 可选 | `testEnvironments.<name>.runtime` | `local-docker` 或 `deployed` |
| `test_environments.<name>.login.credential_ref` | 可选 | `testEnvironments.<name>.login.credentialRef` | 本机安全凭据引用；不保存账号密码 |
| `test_environments.<name>.data` | 可选 | `testEnvironments.<name>.data` | 平台/租户逻辑数据目标、租户解析规则与测试数据约束 |
| `test_environments.<name>.frontend.build` | 可选 | `testEnvironments.<name>.frontend.build` | 本地构建是否必需及命令、工作目录、产物目录 |
| `test_environments.<name>.desc` | 可选 | `testEnvironments.<name>.desc` | 测试环境说明 |
| `apifox.projects.<repo>.project_id` | 可选 | `apifox.projects.<repo>.projectId` | 该仓/接口域使用的 Apifox 项目 ID |
| `apifox.projects.<repo>.branch` | 可选 | `apifox.projects.<repo>.branch` | Apifox 分支；缺省 `main` |
| `apifox.projects.<repo>.environments.local` | 可选 | `apifox.projects.<repo>.environments.local` | 本地接口测试环境（与 test 分离） |
| `apifox.projects.<repo>.environments.test` | 可选 | `apifox.projects.<repo>.environments.test` | 测试环境（与 local 分离） |
| `apifox.routes.<name>` | 可选 | `apifox.routes.<name>` | 接口前缀/受影响仓库到 Apifox 项目的路由 |

\* `project_id` 与 `project_path` 至少给一个；都给以 `project_id` 为准；都不给 → 解析器抛 `missing required fields`。

`jenkins.*` 启用条件：`job_name` 或 `jobs` 任一存在即启用 Jenkins 能力（`default_params` 缺省为空表，`branch_param` 缺省 `"branch"`）；两者皆空则不启用。`roles` / `databases` / `test_environments` 都是命名映射，按需列出每项；整个键缺省则该能力不可用。
