...
GateSet 的逻辑环境当前只有 `local` / `test`。引擎只在当前转换所需环境缺少 AssetAudit/TestRun（或 full 回归证据缺失）时生成 `run_affected_regression` / `run_full_regression`；已有环境证据时不生成回归动作。生成的动作使用 `test-flow-e2e`，执行后必须用 `pnpm cli du` 的 `record` 把 TestRun/AssetAudit 记入 DU，并重新运行 `transition`。开发中→测试中的 local 回归必须排在 feature commit 之后、merge/deploy 之前。
