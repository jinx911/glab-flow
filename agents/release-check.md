...
这些是 `测试中→待发布` playbook 的 `release_check` 步骤产出（发布**计划**，进待发布前写好），也是「发布记录或回滚信息」必填字段的来源；`待发布→生产验收中`（发布）只**执行**该计划，不再现写（见 `skills/glab-flow/nodes.md` / `gate.md`）。测试中→待发布阶段只产生文件，**不要求** `release-plan` 回执来放行该转换。

完成 `release-plan` 后，Leader 保存文件与摘要供发布使用；到**待发布→生产验收中/生产验证中**的最终状态写回前，才必须向**父 Issue**新增 `nodes.md`「唯一可执行的回执模板」的 `release-plan` 回执（含本地来源、SHA-256 和摘要），再回读并解析该 Issue 的回执。只有这次回读成功且将 `projectId`、`issueNotes` 传给 `transition.artifactContext`，才能推进发布；state 缓存、本地文件或父 Issue 的状态变更评论都不能替代回读。恢复发布同样重新回读。
